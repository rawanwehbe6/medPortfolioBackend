...
const updateForm = async (req, res) => {
    try {
        ...
        let instructor_signature = null;

        // If a file for instructor_signature was uploaded, process it
        if (req.file) {
            instructor_signature = req.file.path.replace(/\\/g, '/'); // Store the file path or handle it as needed
        }
        ...
        const [result] = await pool.execute(
            ...
            [
                ...
                sanitize(communication), sanitize(history_taking), sanitize(physical_examination), sanitize(clinical_reasoning),
                sanitize(application_knowledge), sanitize(overall_marks), sanitize(strengths), sanitize(suggestions),
                sanitize(instructor_signature), sanitize(sent), sanitize(completed),
                sanitize(id)
            ]
        );
