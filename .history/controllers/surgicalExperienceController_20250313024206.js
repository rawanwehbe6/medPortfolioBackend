...
const addSurgicalExperience = async (req, res) => {
  try {
    ...
    await pool.execute(
      "INSERT INTO surgical_experiences (User_ID, ProcedureName, Date, Role, Clinic, Description) VALUES (?, ?, ?, ?, ?, ?)",
...
const updateSurgicalExperience = async (req, res) => {
  try {
    ...
    await pool.execute(
      "UPDATE surgical_experiences SET ProcedureName = ?, Date = ?, Role = ?, Clinic = ?, Description = ? WHERE id = ? AND User_ID = ?",
      [procedureName, date, role, clinic, description, id, userId]
    );
