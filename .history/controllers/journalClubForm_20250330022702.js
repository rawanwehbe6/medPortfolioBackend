...
const upload = require("../middlewares/mullterConfig");
const fs = require("fs");
...
const updateAssessment = async (req, res) => {
    try {
        upload.fields([
            ...
            const residentSignaturePath = req.files["resident_signature"] ? req.files["resident_signature"][0].path : existingRecord[0].resident_signature;
            const assessorSignaturePath = req.files["assessor_signature"] ? req.files["assessor_signature"][0].path : existingRecord[0].assessor_signature;

            if (req.files["resident_signature"] && existingRecord[0].resident_signature) {
                fs.unlinkSync(existingRecord[0].resident_signature);
            }
            if (req.files["assessor_signature"] && existingRecord[0].assessor_signature) {
                fs.unlinkSync(existingRecord[0].assessor_signature);
            }
            ...
            await pool.execute(
                ...
                [resident_name, article_reference, paper_selection, background_knowledge,
                    ...
                    suggested_article_selection, suggested_critical_analysis, suggested_slide_design,
                    suggested_presentation, suggested_answering_questions, agreed_action_plan,
                    residentSignaturePath, assessorSignaturePath, id]
            );
