...
const addSurgicalExperience = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Server error during surgical experience creation" });
  }
};
...
const updateSurgicalExperience = async (req, res) => {
  try {
    ...
    if (!userId || !id) {
      return res
        .status(400)
        .json({ error: "Missing required fields: userId or id" });
    }
    ...
    if (existingExperiences.length === 0) {
      return res.status(404).json({
        error: "Surgical experience not found or does not belong to the user.",
      });
    }
...
`;
    ...
    res
      .status(200)
      .json({ message: "Surgical experience updated successfully" });
  } catch (err) {
    console.error("Database Error:", err);
    res
      .status(500)
      .json({ error: "Server error during surgical experience update" });
  }
};
